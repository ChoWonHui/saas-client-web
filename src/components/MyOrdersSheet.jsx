...
const isPast = (o) => o.status === 'CLOSED' || o.status === 'CANCELLED'

function time(dt) { return dt ? dt.slice(11, 16) : '' }

/** 내 주문 내역 — 진행 중 주문과 지난(완료·취소) 내역을 함께 보여준다(읽기 전용, 부모가 주기적으로 갱신). */
export default function MyOrdersSheet({ orders, onClose }) {
  const { tr, L } = useI18n()
  const active = orders.filter((o) => !isPast(o))
  const past = orders.filter(isPast)
  // 합계는 취소를 뺀 실제 주문 금액만.
  const total = orders.filter((o) => o.status !== 'CANCELLED').reduce((s, o) => s + o.totalAmount, 0)
        ...
        <div className="sheet-body">
          {orders.length === 0 ? (
            ...
            <>
              {active.length > 0 && (
                <>
                  <div className="myo-group">{L('myoActive')}</div>
                  {active.map(card)}
                </>
              )}
              {past.length > 0 && (
                <>
                  <div className="myo-group">{L('myoPast')}</div>
                  {past.map(card)}
                </>
              )}
            </>
